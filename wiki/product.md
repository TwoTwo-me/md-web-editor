...
9. Optional discovery of all ordinary file extensions in the explorer, quick switch and wiki completion; hidden/VCS exclusions remain. New wiki links store a selected-folder-root target such as `[[/자료/설명서.pdf|설명서]]`, never a machine absolute path. Existing untouched links are preserved. Attachments open in a local dialog: raster images and plain text can be previewed; other formats can be downloaded for opening with a device application. HTML/SVG/code previews remain text, and no remote viewer is used.

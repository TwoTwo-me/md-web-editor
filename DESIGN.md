...
Graph refinement: the canvas owns the full flexible center track. Settings and the keyboard-accessible note list open as bounded overlays from labeled toolbar buttons, rather than permanently narrowing the graph. A compact modification-history strip sits below the canvas. Notes remain reachable through the list toggle on every viewport. Rendered editor blocks explicitly use normal whitespace; source uses pre-wrapped CodeMirror text.

Attachment interaction: Settings has an optional all-files listing toggle using the existing checkbox primitive. It exposes file names in the explorer, quick switch and wiki completions. Non-Markdown links open a native dialog showing vault-relative path, file size, safe image/plain-text preview where supported, and an explicit download action. HTML/SVG/code are text, never executable surfaces; PDF/office/archive/unknown binary files use the download action. Preview stays within the existing dialog height/scroll region at phone sizes, reports loading/read errors, supports Escape and restores focus. No new palette or spacing tokens.

## 9. Tab groups and split panels
Reference: Obsidian's documented tab groups support moving tabs between groups and splitting right/down; edge drops create splits. The implementation follows that interaction grammar within the existing editor shell. Existing file explorer and inspector remain collapsible sidebars. Main content uses recursive horizontal/vertical splits with one independent tab bar, toolbar and active view per group. Notes and global/local graphs are peer tabs. A focused group has a subtle accent edge; unfocused active tabs retain readable selection without competing accents.

Tabs support reorder, transfer to another group, and left/right/top/bottom edge drop previews. A tab menu and group menu expose the same actions for keyboard and touch: move right/down, duplicate in a right/down split, choose another group, close tab/group, and merge groups. Separators have a 6px pointer target, visible hover/focus accent, keyboard arrow adjustment, Home/End limits and double-click equal split. Drag highlights use existing accent-soft/accent tokens and contain no file content. No layout animation during typing, dragging or resizing. Menus use 140ms opacity only, arrow/Home/End navigation, Escape dismissal and focus restoration; mechanism adapted from beui.dev context-menu source (2026-09-13).

Each note view keeps its own mode, selection and scroll position while sharing one document and autosave queue per path. Moving a tab reuses the live view and graph simulation. Graph navigation opens a note in an existing note group when possible so the graph stays visible. Below a 760px window or 600px available workspace width, groups remain available through a labeled group switcher and only the selected group occupies the content area; resizing back restores the split geometry. Split-group controls wrap according to the group's available width, not only the window width. Layout metadata is persisted locally per vault, restored only after the folder is reopened, and never stores document contents or machine absolute paths.

## 10. Installed application
Chrome owns the native installation dialog and standalone window frame. The installed identity stays `md-web-editor`, with the existing boxed `.md` mark at 192/512px and dark panel background. No simulated browser controls or custom installation modal. The welcome help uses its existing text primitive for an aria-live offline preparation state: preparing, ready, or failed. Ready appears only after a worker completes installation; initial preparation failure stays visible. The existing save-before-update action remains explicit. Online access is needed for initial download and updates, while an installed, cached app opens local folders and saves without internet. Clearing browser app data removes the cached application, not the selected folder's original files.
